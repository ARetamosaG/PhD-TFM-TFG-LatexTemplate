...
La tabla~\ref{tab:complejidades} recoge las complejidades temporales más habituales. A partir de ella, se pueden establecer 
tres clases abstractas de problemas, según su complejidad: P, NP y NP-completo. Los problemas de tipo P\footnote{
\textit{Polynomial time}.} son aquellos que se pueden resolver aplicando un algoritmo en tiempo polinómico, esto es, 
desde $O(1)$ hasta $O(n^2)$\footnote{Para los casos de complejidad polinómica y exponencial, se utiliza como exponente y base
respectivamente la constante 2, cuando realmente puede ser cualquier número entero. Por lo tanto, la complejidad polinómica se 
debería notar como $O(n^p)$, y la exponencial como $O(p^n)$, siendo \textit{p} un entero.}. Los problemas de tipo NP\footnote{
\textit{Non Polynomial time}.} son aquellos con una complejidad mayor a la polinómica pero inferior a la exponencial, cuya
resolución suele basarse en un enfoque híbrido entre un algoritmo y una heurística. Finalmente, los problemas de tipo NP-completo
son aquellos con una complejidad superior a la exponencial. Además, no cuentan con ningún algoritmo eficiente que corra en tiempo
polinómico.

El foco de atención de este trabajo va a ser esta última clase de problemas, los NP-completos, y su resolución a través de 
diferentes técnicas heurísticas y metaheurísticas, equilibrando el balance entre la eficiencia computacional de la resolución, 
y la proximidad de dicha solución al óptimo. Se va a tomar el problema del viajante como referencia de problema NP-completo.
